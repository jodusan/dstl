...
from utils import N_Cls, M, stretch_n, SB, inDir, GS, combined_images, DF
...
def predict_image(id, model, trs):
    ...
    Returns:
    - prd: predictions for image with (id), with shape of (N_Cls, image_size, image_size)
    """
...
#
#     for i, id in test_imgs:
#         print "[predict_test_images] Predicting image #",i, " id",  id
#         msk = predict_image(id, model, trs)
#         #np.save('views/test/10_%s' % id, msk)


def make_test_patches(id, model, trs, label):
    img = combined_images(id, image_size)
    x = stretch_n(img)
