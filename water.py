...
def CCCI_index(id, idx1, idx2):
    ...
    RE = resize(m[idx1, :, :], (rgb_image.shape[0], rgb_image.shape[1]))
    MIR = resize(m[idx2, :, :], (rgb_image.shape[0], rgb_image.shape[1]))
    #RE = rgb_image[:, :, 1]
    #RE = RE*1.0/np.amax(RE)
    #print RE
    # canopy chloropyll content index
    #CCCI = (MIR - RE) / (MIR + RE) * (MIR - R) / (MIR + R)
    #CCCI = ((MIR*1.0) - RE) / (MIR+RE*1.0) * (MIR*1.0-R) / (MIR*1.0 + R)
    CCCI = (RE*1.0-MIR*1.0)/(RE*1.0+MIR*1.0)


    CCCI = (CCCI > 0.12).astype(np.float32) # binary
    # print RE
    # print MIR
    # print R
    return resize(CCCI, (256, 256))
...
def waterway():
    img_idxs = ['6100_2_2', '6100_1_2', '6100_0_2', '6080_4_4', '6080_4_3', '6080_4_2', '6080_4_1', '6080_4_0']
    #img_idxs= ['6080_4_3']
    arr_idxs = [0, 1, 2, 3, 4, 5, 6, 7]

    #img = CCCI_index('6100_2_2')
    #print img
    #cv2.imwrite('bla.png', img*255)
    for img_idx in img_idxs:
        for arr_idx in arr_idxs:
            for sec_idx in arr_idxs:
                if sec_idx == arr_idx:
                    continue
                #print bla," Arr: ", arr_idx, " Sec: ", sec_idx
                directory = 'test/'+img_idx
                if not os.path.exists(directory):
                    os.makedirs(directory)
                cv2.imwrite(directory+'/'+img_idx+'-'+str(arr_idx)+'-'+str(sec_idx)+'.png', CCCI_index(img_idx, arr_idx, sec_idx)*255)
                #combinations.append(CCCI_index(img_idx, arr_idx, sec_idx))
